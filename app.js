...
function loginUser(user) {
    AppState.user = user;
    AppState.points = user.points || 0;
    AppState.history = user.history || [];
    AppState.stats = user.stats || { totalEarned: 0, bottlesRecycled: 0 };
...
function handleLogout() {
    if (confirm('¿Estás seguro de cerrar sesión?')) {
        ...
        AppState.points = 0;
        AppState.history = [];
        AppState.stats = { totalEarned: 0, bottlesRecycled: 0 };
...
function showView(viewName) {
    ...
    // Highlight active nav item based on view
...
function simulateScan() {
    ...
    setTimeout(() => {
        ...
        button.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg> Simular Escaneo';
    }, 2000);
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (currentUser) {
        ...
        // Reload user data from users array
        const users = JSON.parse(localStorage.getItem('ecopoints_users') || '[]');
